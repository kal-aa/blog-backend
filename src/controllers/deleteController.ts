...
export const deleteComment: ReqResNext = async (req, res, next) => {
  const { id } = req.params;
...
export const deleteReply: ReqResNext = async (req, res, next) => {
  const { id } = req.params;
...
export const deleteBlog: ReqResNext = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) return constErr(404, "User not found", next);
...
export const accountDelete: ReqResNext = async (req, res, next) => {
  ...
  try {
    ...
    if (result.deletedCount === 0) return constErr(404, "User not found", next);
